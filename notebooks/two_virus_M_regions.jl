...
Ms = let
	_Ms = @bind M Slider([1,2,3,4,5,10,20], show_value=true, default=2)
	md"M = $(_Ms)"
end
...
Cs = let
	...
	_Cs = @bind c Slider(c_values, show_value=true, default = c_values[end])
	md"c = $(_Cs)"
end

# ╔═╡ f431ba40-9742-4898-92f0-33c3b8e12a1c
params_one, params_many = let
	N = 2
	α = 3
	γ = .005
	PSS.Parameters(; N, M=1, α, γ, c), PSS.Parameters(; N, M, α, γ, c)
end;
...
one_group, many_groups = let
	...
	R = [PSS.Region(; viruses=[v1,v2], K=K0) for m in 1:params_many.M]
	R[1] = special_group
...
# ╔═╡ 018f0e4f-0a0f-4840-895a-cd1e81d2f1c6
state_init_many
...
p_freq_many = let
	...
	hline!(
		...
		label="", line=(:gray, 10, .5)
	)
	annotate!(T*.6, .08, ("M=$M immune groups", 28))
...
p_freq_one = let
	...
	annotate!(T*.6, .08, ("One immune group", 28))
	# plot logistic growth for comparison
	plot!(tvals, logistic_growth(
...
let
	panel = plot(
		pI_one, pS_one, p_freq_one, pI_many, pS_many, p_freq_many;
		layout = grid(2, 3),
		size = (2400, 1600),
		bottom_margin = 10mm, 
		right_margin = 10mm,
...
# ╔═╡ efa33f9f-972b-406c-98de-d8330ba458f2
md"""
Use like this:
```
font_size = 20
...
# ╔═╡ 0ce3894d-acf6-4618-bb1e-33be815e6ef2
