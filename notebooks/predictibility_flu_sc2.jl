...
# ╔═╡ ae7c61cc-c408-11ee-05d6-6f8b09d1270f
begin
	using Chain
	using JSON3
	using LaTeXStrings
...
# ╔═╡ c9b84c0c-0712-4ca0-998b-909b46a527f5


# ╔═╡ 04664175-3f7c-41b6-83ae-eb8ca1aa32f7
...
# ╔═╡ ab354e45-17d7-4345-94ff-494577e6caf8
begin
	plt_defaults = pubfig(24)
	Plots.default(; plt_defaults...)
end

# ╔═╡ 734b2a26-a639-4c85-bb37-a8fd1414bb98
begin
	dat_h3n2 = open(io -> JSON3.read(io, Dict), "h3n2_ha.json", "r")
	dat_sc2 = open(io -> JSON3.read(io, Dict), "sc2.json", "r")
end

# ╔═╡ d0abcf2f-be45-4c18-9277-2049efa230ac
begin
	dat_h3n2["time_points"]
	keys(dat_h3n2["trajectories"]) |> collect
end

# ╔═╡ cb04eaba-1de2-4118-8915-9e1fc1edbc4b
dat_h3n2["trajectories"]["0.4"]

# ╔═╡ 262b3a8e-db22-47a3-a5db-7402afdfe3bb
dat_h3n2["trajectories"] |> keys |> collect |> sort
...
begin
	ann_flu = ("H3N2/HA", 32, :left, "Helvetica Bold")
	ann_sc2 = ("SARS-CoV-2", 32, :left, "Helvetica Bold")
end

# ╔═╡ 23c417e5-a587-4c69-a747-8d390bd140cc
range(-7, 25) * 7 |> collect 

# ╔═╡ ed292588-6805-4e82-a221-20459612b8a2
function inertia_many(dat; sc2=false, kwargs...)
	tvals = if !sc2
		dat["time_points"]
	else
	...
	fbins = dat["trajectories"] |> keys |> collect
	...
	pal = palette(:roma, length(dat["trajectories"]), rev=true)


	for (i, fbin) in enumerate(dat["trajectories"] |> keys |> collect |> sort)
...
# ╔═╡ 627c3779-aa84-4df1-ab25-542d11851a12
inertia_many(dat_sc2; sc2=true)

# ╔═╡ 41cd53e1-1c04-4b97-849b-2ef40cd3f2a1
dat_h3n2["trajectories"] |> keys |> collect |> sort

# ╔═╡ ec8aa89b-5f1d-47bd-93ee-44de316ac9b0
...
function inertia_one(dat; fbin = 0.4, sc2=false, write_ann=true, kwargs...)
	...
	for T in values(dat["trajectories"][string(fbin)])
		color = clr_from_final(T, .2)
		plot!(tvals, T; label="", line=(2, .3, color))
	end
...
let
	p = plot(
		inertia_one(dat_h3n2; ann=(-400, .95, ann_flu), xlim=(-450, 1050)), 
		inertia_many(dat_h3n2),
		inertia_one(dat_sc2; sc2=true, ann=(-80, .95, ann_sc2), xlim=(-90, 160)), 
		inertia_many(dat_sc2; sc2=true);
		size = (2400, 2000),
		layout = grid(2,2),
		dpi = 300,
